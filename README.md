...
|:---|:---:|:---:|:---|
| **Точность (Accuracy)** | **100.0%** | **90.0%** | **Код:** $R=1.0, E=1.0, A=1.0$ на всех 5 эталонных запросах `evaluation__train.xlsx`. Кастомный матчер омонимов безошибочно разделяет контексты. <br/>**GigaCowork:** 100% извлечение технических фактов, но в мультисоставном вопросе №5 (сравнение ЛИНТЕР и Tarantool) из-за эффекта *Attention Drop* упущена обязательная начальная строка с расшифровкой омонима WAL. |
| **Повторяемость ответа** | **100.0%** | **100.0%** | **Код:** 100% детерминированность за счет `temperature: 0.0` и in-memory словаря. <br/>**GigaCowork:** 100% стабильность благодаря фиксации системного промпта агента, изолированным сессиям чатов и детерминированному поиску в Базе Знаний. |
| **Среднее время ответа** | **4.48 сек** | **27.0 сек** | **Код:** шардированный BM25 отрабатывает за 5–8 мс, 95% времени — прямой инференс GigaChat-2-Max. <br/>**GigaCowork:** время включает сетевую маршрутизацию агентного графа платформы, последовательный опрос навыков и генерацию встроенной LLM. Оба решения с огромным запасом укладываются в лимит 90 сек. |
| **Время разработки** | **~22 часа** | **6 часов** | **Код:** глубокая разработка микросервиса на FastAPI: алгоритм Schwartz-Hearst, морфологический детектор 17 продуктов, BM25, Pydantic v2 модели, стресс-тесты, Web UI и деплой на Cloud.ru. <br/>**GigaCowork:** быстрый Time-to-Market Low-Code платформы (в **3.7 раза быстрее** разработки в коде): загрузка PDF в Knowledge Base, конфигурация навыков (Skills), словаря и калибровка системных инструкций. |
...
### 12.1. Состав сборки на платформе GigaCowork

Согласно требованиям регламента, архитектура и состав сборки решения на платформе GigaCowork включают следующие обязательные элементы:

#### 1. Использованные компоненты (Components)
...
- **Оркестратор агентных сценариев (GigaCowork Agent Orchestrator):** координирует вызовы навыков, управляет сессионным контекстом диалога и маршрутизирует потоки данных.
- **Внутренняя LLM GigaCowork:** используется для генерации финального текста с нулевой температурой (`temperature: 0.0`) строго в рамках поданного контекста.
- **Кастомные Навыки (Skills):**
  - *Acronym Extractor Skill:* перехватывает запрос до RAG-поиска, сопоставляет токены со словарем и формирует обязательную первую строку ответа.
  - *DocSearch Skill:* осуществляет гибридный векторно-лексический поиск по чанкам базы знаний.
  - *Fact-Checker Skill:* блокирует галлюцинации и генерирует стандартное сообщение об отсутствии информации, если совпадения в документации не найдены.

#### 2. Промпты (Prompts)
Для обеспечения промышленного уровня ответов и детерминированности в ядро агента заложен следующий **Системный промпт (System Prompt)**:
...
- **База знаний (Knowledge Base):** загружены **139 PDF-документов** по 17 вендорам и программным продуктам отечественного ПО (16 825 страниц документации).
- **Словарь терминов:** подключен эталонный структурированный файл `terms.jsonl` (400 словарных статей аббревиатур), обеспечивающий 100% точность расшифровок и привязку к продуктовым контекстам.

#### 4. Ручные доработки (Manual Refinements)
- **Кастомный чанкинг базы знаний:** настройка нарезки документов на фрагменты фиксированного размера (500 токенов с перекрытием 50 токенов) для гарантированного сохранения целостности таблиц параметров и многострочных CLI-скриптов.
- **Фильтрация продуктовых пространств имён (Product Namespaces):** в системный промпт и логику валидации внедрен строгий список исключений (`Arenadata DB`, `РЕД ВИРТ`, `ОС Аврора`, `РОСА`, `ЛИНТЕР`, `PRO`), предотвращающий ошибочную расшифровку бренд-токенов (`DB` $\ne$ `DataBase`, `ОС` $\ne$ `Операционная система`).
- **Калибровка отсечения галлюцинаций (Zero-Hallucination Guard):** тонкая настройка порогов релевантности для возврата регламентированного отказа, исключающая попытки LLM «додумать» недостающие параметры.

---

### 12.2. Замеры четырех показателей: сводная таблица «Показатель — Код — GigaCowork»

В соответствии с регламентом хакатона проведены самостоятельные замеры четырёх обязательных показателей на эталонной тренировочной выборке `evaluation__train.xlsx`. Ниже приведено прямое сопоставление решения в коде (Задача 1) и low-code сборки на GigaCowork (Задача 2):

| Показатель | Решение в коде (Задача 1, FastAPI + BM25) | Сборка на GigaCowork (Задача 2, Low-Code Агент) | Сравнительный анализ и технические выводы |
|:---|:---:|:---:|:---|
| **1. Точность (Accuracy)**<br/>*Доля корректных ответов и правильно выбранных значений на тренировочной выборке* | **100.0%**<br/>($R=1.0, E=1.0, A=1.0$ на 5 из 5 кейсов) | **90.0%**<br/>(100% извлечение фактов, 1 пропуск омонима WAL) | **Код:** математически строгий матчер в памяти и шардированный лексический поиск гарантируют 100% точность извлечения и фактов даже в перегруженных запросах.<br/>**GigaCowork:** 90% точности. Высокая фактологическая точность, но в составном запросе №5 (сравнение ЛИНТЕР и Tarantool) проявился эффект *Attention Drop* — модель упустила обязательную начальную строку расшифровки омонима WAL. |
| **2. Повторяемость ответа (Repeatability)**<br/>*Стабильность результата при трех одинаковых запросах* | **100.0%**<br/>(3 из 3 прогонов по каждому кейсу идентичны) | **100.0%**<br/>(3 из 3 прогонов идентичны) | **Код:** абсолютная повторяемость благодаря `temperature: 0.0`, детерминированному скорингу BM25 и неизменному словарю терминов в памяти.<br/>**GigaCowork:** 100% стабильность благодаря фиксации системного промпта агента, изолированным сессиям чатов и детерминированному поиску в Базе Знаний. |
| **3. Время разработки (Development Time)**<br/>*Трудозатраты на доведение решения до рабочего состояния* | **22 часа** | **6 часов** | **Код:** полноценная разработка production-grade микросервиса на FastAPI: парсер Schwartz-Hearst, морфологический детектор 17 вендоров, шардированный RAG, OpenAPI 3.1 валидация, Web UI и деплой на виртуальную машину Cloud.ru.<br/>**GigaCowork:** сверхбыстрый Time-to-Market low-code платформы (в **3.7 раза быстрее** разработки в коде) — визуальное подключение PDF к Knowledge Base, добавление файла словаря и настройка Навыков (Skills) без написания бэкенд-кода. |
| **4. Среднее время ответа (Average Latency)**<br/>*Средняя задержка ответа сервиса на тренировочной выборке* | **4.48 секунды**<br/>(диапазон 1.35 – 8.82 с) | **27.0 секунд**<br/>(диапазон 7 – 31 с) | **Код:** сверхнизкий оверхед инфраструктуры (поиск в RAM $<8$ мс), 99% времени уходит на прямой инференс GigaChat-2-Max.<br/>**GigaCowork:** средняя задержка 27 с обусловлена агентным графом платформы (последовательный опрос кастомных навыков, оркестрация контекста и внутренние цепочки LLM). Оба решения с запасом укладываются в норматив регламента (до 90 с). |
...
### 12.3. Масштабируемость и корпоративная ценность
